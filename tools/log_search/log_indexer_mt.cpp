...
#include <terark/thread/pipeline.cpp> //Makefile:LDFLAGS:-lboost_thread -lboost_system
#include <getopt.h>
...
struct ParsingThreadContext {
	std::unique_ptr<MultiRegexSubmatch> m_submatch;
	long long m_parseTime;
	ParsingThreadContext() : m_parseTime(0) {}
};

struct IndexOneDocTask : public PipelineTask {
	std::string m_content;
	NativeDataOutput<AutoGrownMemIO> m_keyValue;
	IndexOneDocTask(const char* data, size_t size) {
		m_content.assign(data, size);
		m_keyValue.resize(size);
	}
	void parse(ParsingThreadContext& ctx) {
		if (m_profilingParseTime) {
			long long t0 = pf.now();
			do_parse(*ctx.m_submatch);
			long long t1 = pf.now();
			ctx.m_parseTime += t1 - t0;
		}
		else {
			do_parse(*ctx.m_submatch);
		}
	}
	void do_parse(MultiRegexSubmatch& submatch);
};

int parseCommandLine(int argc, char* argv[]) {
	for (;;) {
		int opt = getopt(argc, argv, "b:r:d:f:t:T:h");
		switch (opt) {
		default:
		case '?':
			usage(argv[0]);
		...
		case 't':
			...
			break;
		case 'T':
			g_parsingThreads = atoi(optarg);
			g_parsingThreads = std::max(g_parsingThreads, 1);
			break;
		case 'h':
			usage(argv[0]);
			exit(0);
...
byte_t m_paddingForFalseSharing1[128];
valvec<ParsingThreadContext> m_threadsContext;
byte_t m_paddingForFalseSharing2[128];
...
void firstPassOneDocStage1(PipelineStage*, int threadno, PipelineQueueItem* qitem) {
	IndexOneDocTask* task = (IndexOneDocTask*)(qitem->task);
	assert(size_t(threadno) < m_threadsContext.size());
	task->parse(m_threadsContext[threadno]);
}

void firstPassOneDocStage2(PipelineStage*, int threadno, PipelineQueueItem* qitem) {
	IndexOneDocTask* task = (IndexOneDocTask*)(qitem->task);
	assert(0 == threadno);
	...
	{
		NativeDataInput<MinMemIO> dio;
		dio.set(task->m_keyValue.begin());
		byte_t* end = task->m_keyValue.current();
		while (dio.current() < end) {
			dio >> m_fieldName;
			dio >> m_fieldValue;
			m_docFields[m_fieldName][m_fieldValue]++;
		}
	}
	for(size_t i = 0; i < m_docFields.end_i(); ++i) {
	...
	m_compactDoc.fromHash(m_docFields);
	docTmpOutput << m_compactDoc;
}
...
public:
int build() {
	for (int i = 0; i < g_parsingThreads; ++i) {
		m_threadsContext.emplace_back();
		m_threadsContext.back().m_submatch.reset(new MultiRegexSubmatch(g_regex_dfa_file, g_regex_binmeta_file));
	}
	...
	for (size_t i = 0; i < m_specVec.size(); i += 3) {
		...
		pipeline
		   	| PPL_STEP_0(this, InvertIndexBuilder, firstPassOneDocStage1, g_parsingThreads)
		   	| PPL_STEP_0(this, InvertIndexBuilder, firstPassOneDocStage2, 0)
		//	| PPL_STEP_0(this, InvertIndexBuilder, firstPassOneDocStage3, 1)
			;
		pipeline.compile();
		terark::LineBuf line;
		while (line.getline(fp) > 0) {
			...
			pipeline.send(new IndexOneDocTask(line.begin(), line.size()));
	...
	for (size_t i = 0; i < m_threadsContext.size(); ++i) {
		parseTime += m_threadsContext[i].m_parseTime;
	}
	if (m_profilingParseTime) {
		fprintf(stderr, "parse[time: %f's  ThroughPut: %f'MB]\n",
				pf.sf(0, parseTime), byteCount/pf.uf(0, parseTime));
...
void IndexOneDocTask::do_parse(MultiRegexSubmatch& submatch) {
	const char* beg = m_content.data();
	const char* end = m_content.size() + beg;
	const char* pos = beg;
	while (pos < end) {
		int maxMatchLen = submatch.match_utf8(fstring(pos, end), gtab_ascii_tolower);
		...
		//		break;
			...
			//	m_docFields["ip"][fstring(pos, maxMatchLen)]++;
				m_keyValue << "ip" << fstring(pos, maxMatchLen);
				break;
			case RegexCate::date_yyyy_m_d_H_M_S:
				...
				*/
				//	m_docFields["datetime"][fstring(pos, maxMatchLen)]++;
					m_keyValue << "datetime" << fstring(pos, maxMatchLen);
				}
				break;
