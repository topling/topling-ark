...
#include <terark/io/var_int_boost_serialization.hpp>
...
#else
 	template<class DataIO> void dio_load(DataIO& dio)	    { dio &a&b&c&d; }
 	template<class DataIO> void dio_save(DataIO& dio) const { dio &a&b&c&d; }
...
{
	int maplen = v3.size();
	printf("%s: loop=%d, time[terark, boost, ratio=b/f] in us\n", prefix, loop);
...
{
	FileStream file0("terark.bin", mode);
	file0.disbuf();
