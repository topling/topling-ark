...
/// --- By rockeet@gmail.com, LeiPeng
/// based on <boost/regex/pending/unicode_iterator.hpp>
/// Optimized for utf8_byte_count, using __builtin_clz
///
...
#ifndef __TERARK_UTIL_UNICODE_ITERATOR_HPP__
#define __TERARK_UTIL_UNICODE_ITERATOR_HPP__
...
#include <stdio.h>
	...
	#define UNICODE_throw_exception boost::throw_exception
#else
	#define UNICODE_throw_exception(e) throw e
#endif
...
struct UTF8_FirstByteKind {
    enum Enum {
       ASCII = 24, // 0xxxxxxx
        TAIL = 25, // 10xxxxxx
          B1 = ASCII,
          B2 = 26, // 110xxxxx 10xxxxxx
          B3 = 27, // 1110xxxx 10xxxxxx 10xxxxxx
          B4 = 28, // 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
          B5 = 29, // 111110xx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
          B6 = 30, // 1111110x 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx
          B7 = 31, // 11111110 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx 10xxxxxx // Error, this is not standard
    } val;
    explicit UTF8_FirstByteKind(unsigned char firstByte) {
        int c = 0xFFFFFF00|firstByte;
        val = (Enum)__builtin_clz(~c|1);
    }
	...
	static bool is_tail(unsigned char c) { return (c & 0xC0) == 0x80; }
};

inline unsigned utf8_byte_count(boost::uint8_t c) {
	static const unsigned char bc[] = { 1, 1, 2, 3, 4, 5, 6, 7 };
	int cx = 0xFFFFFF00|c;
	int n = __builtin_clz(~cx|1);
//	assert(25 != n);
	return bc[n-24];
}
...
#ifndef BOOST_NO_STD_LOCALE
   char buf[256];
   int len = sprintf(buf, "Invalid UTF-32 code point U+0x%X encountered while trying to encode UTF-16 sequence", val);
   std::string msg(buf, len);
#else
   std::string msg("Invalid UTF-32 code point encountered while trying to encode UTF-16 sequence");
#endif
   std::out_of_range e(msg);
   UNICODE_throw_exception(e);
}
#ifdef BOOST_MSVC
#pragma warning(pop)
#endif
...
#ifndef BOOST_NO_STD_LOCALE
	  char buf[256];
      int len = sprintf(buf, "Misplaced UTF-16 surrogate U+0x%X encountered while trying to encode UTF-32 sequence", val);
	  std::string msg(buf, len);
      std::out_of_range e(msg);
#else
	  std::string msg("Misplaced UTF-16 surrogate encountered while trying to encode UTF-32 sequence");
      std::out_of_range e(msg);
#endif
      UNICODE_throw_exception(e);
   }
...
template<class BaseIterator>
u8_to_u32_iterator<BaseIterator>
make_u8_to_u32_iter(BaseIterator bi) {
	return u8_to_u32_iterator<BaseIterator>(bi);
}
...
template <class BaseIterator>
utf16_output_iterator<BaseIterator>
make_utf16_output_iter(BaseIterator bi) {
	return utf16_output_iterator<BaseIterator>(bi);
}
...
template <class BaseIterator>
utf8_output_iterator<BaseIterator>
make_utf8_output_iter(BaseIterator bi) {
	return utf8_output_iterator<BaseIterator>(bi);
}

} // namespace terark

#endif // __TERARK_UTIL_UNICODE_ITERATOR_HPP__
