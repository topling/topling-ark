...
public:										\
	TERARK_RPC_GEN_full_mf_name(ClassName)	\
...
class TERARK_DLL_EXPORT server_stub_i
{
public:
	std::string m_name;
	unsigned m_id;
...
struct TERARK_DLL_EXPORT ClassMeta : public RoCreator
{
	std::vector<server_stub_i*> stubs;
};

class TERARK_DLL_EXPORT server_packet_base : public RefCounter
{
public:
	server_stub_i* stub;
	unsigned     seqid;
...
#define BOOST_PP_FILENAME_1       <terark/rpc/pp_server_stub.hpp>
...
//////////////////////////////////////////////////////////////////////////
class TERARK_DLL_EXPORT session_base : public SessionScope
{
protected:
...
class TERARK_DLL_EXPORT rpc_server_base
{
...
/*
	class RpcTask : public terark::thread::PipelineTask
	{
		so_input_t* input;
		so_output_t* output;
		server_stub_i* stub;
...
server_stub<Session, Function>::sync_call(session_base* vpsession, const server_packet_base* header) const
{
	...
	refs.dio_load(session->m_so_input);
...
server_stub<Session, Function>::do_send_args(Session* session, packet_t* p) const
{
	...
	aref_t refs(boost::mpl::true_(), p->argvals);
	refs.dio_save(session->m_so_output);
