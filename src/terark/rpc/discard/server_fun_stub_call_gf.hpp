...
public:
	...
	template<class DataIO> void dio_load(DataIO& dio, arglist_val& args) { }
	template<class DataIO> void dio_save(DataIO& dio, arglist_val& args) { }
#else
...
#define GetTupleArgListVal(z, n, _) ArgVal<Arg##n>::deref(get<n>(args))
	...
	void dio_load(DataIO& dio, arglist_val& args)
	{
		using boost::tuples::get;
		arglist_ref refs(BOOST_PP_ENUM(ArgCount, GetTupleArgListVal, ~));
		dio >> refs;
	...
	template<class DataIO>
	void dio_save(DataIO& dio, arglist_val& args)
	{
		using boost::tuples::get;
		arglist_ref refs(BOOST_PP_ENUM(ArgCount, GetTupleArgListVal, ~));
		dio << refs;
