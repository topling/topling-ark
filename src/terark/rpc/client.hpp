...
class TERARK_DLL_EXPORT client_packet_base : public RefCounter
{
public:
...
public:
	...
	void t_read_args(rpc_client_basebase* vpclient, arglist_ref_t& refs) {
		...
		refs.dio_load(client->m_co_input);
	...
	void t_send_id_args(rpc_client_basebase* vpclient, arglist_ref_t& refs) {
		...
		client->m_co_output.get()
			...
			;
		refs.dio_save(client->m_co_output);
...
#define BOOST_PP_FILENAME_1       <terark/rpc/pp_client_stub.hpp>
#include BOOST_PP_ITERATE()
//-------------------------------------------------------------------------
//

class TERARK_DLL_EXPORT rpc_client_base : public SessionScope
{
protected:
