...
struct DenseStateV2 : public StateFlags {
	typedef static_bitmap<Sigma*2, bm_uint_t> MyBitMap;
	enum { PoolAlign = sizeof(StateID) };
	enum { DenseBitIdx = 0, SparseBitIdx = 1 };
	static const StateID nil_state = StateID(-1);
	static const StateID max_state = StateID(-1) - 1;
#ifndef TERARK_WORD_BITS
	#error "Unknow TERARK_WORD_BITS"
#elif TERARK_WORD_BITS == 64
	static const bm_uint_t  DenseMask = 0x5555555555555555ULL;
	static const bm_uint_t SparseMask = 0xAAAAAAAAAAAAAAAAULL;
#elif TERARK_WORD_BITS == 32
	static const bm_uint_t  DenseMask = 0x55555555UL;
	static const bm_uint_t SparseMask = 0xAAAAAAAAUL;
#else
	#error "Invalid TERARK_WORD_BITS" = TERARK_WORD_BITS
#endif
	...
	int num_children() const {
		...
		for (int i = 0; i < MyBitMap::BlockN; ++i) {
			bm_uint_t b =  bits.block(i);
			bm_uint_t d = (b &  DenseMask) << 1;
			bm_uint_t s = (b & SparseMask);
			n += fast_popcount(d | s);
		}
		assert(n <= Sigma);
		return n;
	...
	int idx_sparse(int c) const {
		...
		if (int shift = (2*c) % MyBitMap::BlockBits) {
			bm_uint_t b =  bits.block(n);
			bm_uint_t d = (b &  DenseMask) << 1;
			bm_uint_t s = (b & SparseMask);
			bm_uint_t x = s & (s ^ d);
	...
	bool has_dense_bit() const {
		for (int i = 0; i < MyBitMap::BlockN; ++i) {
			if (bits.block(i) & DenseMask)
				return true;
		}
		return false;
	}

	int dense_popcnt() const {
		int n = 0;
		for (int i = 0; i < MyBitMap::BlockN; ++i)
			n += fast_popcount(bits.block(i) & DenseMask);
		return n;
	}
...
template< class StateID, int Sigma = 256
	    , class State = DenseStateV2<StateID, Sigma>
	   	>
class DenseDFA_V2 : public BaseDFA, public DFA_MutationInterface
{
public:
...
public:
    DenseDFA_V2() : pool(sizeof(StateID)*sigma) { init(); }
    ...
    size_t new_state() override final {
        ...
        if (nil_state == firstFreeState) {
            ...
            s = states.size();
            if (s >= max_state) {
                throw std::runtime_error("DenseDFA_V2::new_state(), exceed State::max_state");
            }
	...
	void set_single_child(state_id_t parent, state_id_t child) {
		...
		assert(states[parent].bits.popcnt() == 1);
    ...
    state_id_t state_move(const State& s, auchar_t ch) const {
		assert(ch < sigma);
		bm_uint_t b = s.bits.block(ch / (sizeof(bm_uint_t)*4))
							   >> (ch % (sizeof(bm_uint_t)*4)) * 2;
		switch (b & 3) {
		default: abort(); break; // avoid warning
		case 0:
			return nil_state;
		case 1: // only dense bit is set
		//	assert(s.dense_target1 < states.size());
			return s.dense_target1;
		case 3: // both dense and spare bits are set
		//	assert(s.dense_target2 < states.size());
			return s.dense_target2;
		case 2: // only sparse bit is set
		  {
	...
	void del_move(size_t s, auchar_t ch) override final {
		...
		State& x = states[s];
		assert(!x.is_free());
		bm_uint_t b = x.bits.block(ch / (sizeof(bm_uint_t)*4))
							   >> (ch % (sizeof(bm_uint_t)*4)) * 2;
		assert(b & 3);
		switch (b & 3) {
		case 0:
			...
			return;
		case 3:
			x.bits.set0(2*ch+1); // clear sparse bit
			// fall through
...
protected:
	...
	//	ASSERT_isNotFree(target);
		...
		if (s.bits.is1(2*ch)) {
			...
			if (s.bits.is0(2*ch+1) && target == s.dense_target1)
				return target;
			if (s.bits.is1(2*ch+1) && target == s.dense_target2)
				return target;
			if (OverwriteExisted) {
				if (s.bits.is1(2*ch+1)) {
					if (target == s.dense_target1) {
						s.bits.set0(2*ch+1); // set target as dense_target1
					...
					if (target == s.dense_target2) {
						s.bits.set1(2*ch+1); // set target as dense_target2
						return s.dense_target1; // old is dense_target1
					}
					old_target = s.dense_target1;
				}
			   	s.bits.set0(2*ch); // will insert into sparse targets
				s.bits.set0(2*ch+1);
				transition_num--;
			} else
				return s.dense_target1;
...
public:
    ...
    void resize_states(size_t new_states_num) override final {
		if (new_states_num >= max_state)
			throw std::logic_error("DenseDFA_V2::resize_states: exceed max_state");
...
#define DenseDFA_V2_compact_leftbrace { // for editor's auto brace match
#if 1
		for (size_t i = 0; i < states.size(); ++i)DenseDFA_V2_compact_leftbrace
			if (states[i].is_free()) continue;
#else
		DFS_GraphWalker<StateID> walker;
		walker.resize(states.size());
		walker.putRoot(initial_state);
		while (!walker.is_finished())DenseDFA_V2_compact_leftbrace
			size_t i = walker.next();
		   	walker.putChildren(this, (StateID)i);
#endif
			State& s = states[i];
			int n_sparse = s.num_sparse();
			assert(s.num_children() == s.dense_popcnt() + n_sparse);
			...
			int n_sparse2 = n_sparse + s.dense_popcnt() - max_freq1 - max_freq2;
			int k = 0;
			int zlen = 0;
	...
	void print_stat(FILE* fp) const {
		...
		for (size_t i = 0; i < states.size(); ++i) {
			...
			bmcnt1[fstring((char*)&s.bits, sizeof(s.bits))]++;
			int d1 = s.num_dense1();
			int d2 = s.num_dense2();
    ...
    void swap(DenseDFA_V2& y) {
		assert(typeid(*this) == typeid(y));
		risk_swap(y);
	}
    void risk_swap(DenseDFA_V2& y) {
		BaseDFA::risk_swap(y);
        pool  .swap(y.pool  );
...
private:
	...
	friend void DataIO_loadObject(DataIO& dio, DenseDFA_V2& au) {
		typename DataIO::my_var_uint64_t version;
		dio >> version;
		if (version > SERIALIZATION_VERSION) {
			typedef typename DataIO::my_BadVersionException bad_ver;
			throw bad_ver(version.t, SERIALIZATION_VERSION, "DenseDFA_V2");
		}
		au.load_au(dio, version.t);
	}

	template<class DataIO>
	friend void DataIO_saveObject(DataIO& dio, const DenseDFA_V2& au) {
...
protected:
	...
	friend size_t
	GraphTotalVertexes(const DenseDFA_V2* au) { return au->states.size(); }
...
const StateID DenseDFA_V2<StateID, Sigma, State>::nil_state;
template<class StateID, int Sigma, class State>
const StateID DenseDFA_V2<StateID, Sigma, State>::max_state;

typedef DenseDFA_V2<uint32_t, 288> DenseDFA_V2_uint32_288;
typedef DenseDFA_V2<uint64_t, 288> DenseDFA_V2_uint64_288;

template<class StateID, int Sigma, class State>
size_t
DenseDFA_V2<StateID, Sigma, State>::hopcroft_hash(size_t x_id) const {
...
template<class StateID, int Sigma, class State>
size_t
DenseDFA_V2<StateID, Sigma, State>::onfly_hash_hash(size_t x_id) const {
	assert(x_id < states.size());
	const State& x = states[x_id];
	assert(!x.is_pzip());
	size_t h = x.is_term();
...
template<class StateID, int Sigma, class State>
bool
DenseDFA_V2<StateID, Sigma, State>::
onfly_hash_equal(size_t x_id, size_t y_id) const {
	...
	for (size_t i = 0; i < MyBitMap::BlockN; ++i) {
		bm_uint_t bx = x.bits.block(i);
		bm_uint_t by = y.bits.block(i);
