...
state_id_t create_max_realnum(fstring max) {
	...
	if (max_i != "0") {
		...
		terark::minimal_sso<32> str_max_i = max_i.str();
...
state_id_t create_lexical_le(fstring max) {
	...
	// 创建“已经小于”状态（接受任意后续字符）
	state_id_t less = _create_any_fraction_state();
	// eq[i] 表示恰好匹配了 max 的前 i 个字符，尚未小于
	std::vector<state_id_t> eq(len + 1);
	eq[0] = root;
	...
	// 构造转移
	for (size_t i = 0; i < len; ++i) {
		unsigned char cur = max[i];
		// 等于当前字符 → 继续精确匹配
		add_move(eq[i], eq[i + 1], cur);
		// 小于当前字符 → 跳转到 less 状态（从此小于 max）
		for (char c = '0'; c < cur; ++c) {
			add_move(eq[i], less, c);
		}
		// 大于当前字符的转移不存在（字符串会大于 max，拒绝）
	}
	// 所有 eq[i] 都是终态（对应 max 的前缀，均 ≤ max）
	for (size_t i = 0; i <= len; ++i) {
		set_final(eq[i]);
	}
	return root;
...
state_id_t create_lexical_ge(fstring min) {
	...
	if (len == 0) { // min 为空：接受所有字符串（包括空串）
		set_final(root);
		add_range_move(root, root, '0', '9');
		return root;
	}
	// 创建“已经大于”状态（接受任意后续字符）
	state_id_t greater = _create_any_fraction_state();
	// eq[i] 表示恰好匹配了 min 的前 i 个字符，尚未大于
	std::vector<state_id_t> eq(len + 1);
	eq[0] = root;
	for (size_t i = 1; i <= len; ++i) {
		eq[i] = new_state();
	...
	// 构造转移
	for (size_t i = 0; i < len; ++i) {
		unsigned char cur = static_cast<unsigned char>(min[i]);
		// 等于当前字符 → 继续精确匹配
		add_move(eq[i], eq[i + 1], cur);
		// 大于当前字符 → 跳转到 greater 状态（从此大于 min）
		for (char c = cur + 1; c <= '9'; ++c) {
			add_move(eq[i], greater, c);
		}
		// 小于当前字符的转移不存在（字符串会小于 min，拒绝）
	}
	// 完全匹配 min 的状态是终态
	set_final(eq[len]);
	// 从完全匹配状态出发，任何额外字符都使字符串大于 min
	for (char c = '0'; c <= '9'; ++c) {
		add_move(eq[len], greater, c);
	}
	return root;
}
