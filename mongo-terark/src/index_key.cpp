...
void terarkEncodeIndexKey(const BSONObj& key, TerarkBsonBlob* encoded) {
	...
	BSONForEach(subkey, key) {
		...
		case Object:
		case mongo::Array:
			invariant(!"terarkEncodeIndexKey(): CodeWScope|Object|Array can not be used as key");
			break;
		case BinData:
		//	valueLen = subkey.valuestrsize() + 4 + 1 /*subtype*/;
		//	can not be BinData, because BinDataSize is required and it can't be lex compared
		//  as binary data
			invariant(!"terarkEncodeIndexKey(): BinData can not be used as key");
			break;
		case RegEx:
			{
				const char* p = value;
		...
		default:
			{
				StringBuilder ss;
				ss << "terarkEncodeIndexKey(): BSONElement: bad subkey.type " << (int)subkey.type();
...
BSONObj terarkDecodeIndexKey(StringData encoded, const BSONObj& fieldnames) {
	...
	while (pos < end) {
		...
		case Object:
		case mongo::Array:
			invariant(!"terarkDecodeIndexKey(): CodeWScope|Object|Array can not be used as key");
			break;
		case BinData:
			invariant(!"terarkDecodeIndexKey(): BinData can not be used as key");
			break;
		case RegEx:
			{
